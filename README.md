...
* **`get_data`**: This function gets all the data from each page we are to parse. It takes in a generator which generates a tuple pair consisting of a page url and page number. We parse for each page, appending the resulting data to the list `master`, and then returning this upon completion. This function's sole responsibility is to _get_ all of the data. It is not responsible for parsing any portions of the data gathered into separate values. 
